...
const AlarmNoticeModel = mongoose.model('alarm_notice');
...
class AlarmManageController {
    static async updateAlaram(ctx, next) {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                AlarmNoticeModel.updateMany({_id: { $in: ids }}, { $set: params }, {} , function(err) {
                    if(err) reject(err);
                    resolve();
                });
            });
    ...
    static async removeAlarams(ctx, next) {
        ...
        await new Promise((resolve, reject) => {
            AlarmNoticeModel.remove({_id:{$in:ids}}, function(err) {
                if(err) reject();
                resolve();
            });
        });
    ...
    static async getRule(ctx, next) {
        ...
        let res = {};
        if(id) {
            const query  = AlarmNoticeModel.where({ _id: id });
    ...
    static async getNoticeList(ctx,next) {
        let messages;
        let count;
        let params = {};
        ...
        await new Promise((resolve, rejcet) => {
            AlarmNoticeModel.find(params, function(err, results) {
                if(err) {
                    process.exit(-1);
                }
                messages = results || [];
    ...
    static async addNotice(ctx,next) {
        const alarm = new AlarmNoticeModel({
            ...
            notice: 'email',
            email: `${Math.floor(Math.random() * 10000000000)}@gmail.com`,
            name: {
                0: '张三',
                1: '李四',
                2: '王二',
