...
class AlarmManageController {
    static async updateAlaram(ctx, next) {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                AlarmMessageModel.updateMany({_id: { $in: ids }}, { $set: params }, {} , function(err) {
                    if(err) reject(err);
                    resolve();
                });
            });
    ...
    static async removeAlarams(ctx, next) {
        ...
        await new Promise((resolve, reject) => {
            AlarmMessageModel.remove({_id:{$in:ids}}, function(err) {
                if(err) reject();
                resolve();
            });
        });
    ...
    static async getAlaram(ctx, next) {
        const id =  ctx.params.id || null;
        let res = {};
        if(id) {
            const query  = AlarmMessageModel.where({ _id: id });
    ...
    static async getAlaramList(ctx,next) {
        ...
        await new Promise((resolve, rejcet) => {
            AlarmMessageModel.find(params, function(err, results) {
                if(err) {
                    process.exit(-1);
                }
                messages = results || [];
